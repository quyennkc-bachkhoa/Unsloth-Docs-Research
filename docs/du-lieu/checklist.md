...
Trang này là danh sách việc cần kiểm tra lại trên dataset ngay trước khi bắt đầu train, để tránh train xong mới phát hiện dữ liệu sai định dạng hoặc thiếu.

::: tip Tóm tắt
- **Dùng khi:** dataset đã gần xong và bạn sắp bấm train.
- **Kết quả:** chắc chắn dataset đúng định dạng, đủ số lượng, đã áp đúng chat template và tránh được các lỗi hay gặp.
- **Nên biết trước:** [Dữ liệu](/du-lieu/), [Định dạng dữ liệu](/du-lieu/dinh-dang), [Chat template](/du-lieu/chat-template).
:::

## Danh sách rà lại trước khi train

Đi lần lượt từng mục dưới đây; mục nào chưa đạt thì quay lại trang tương ứng.
...
3. Đủ số lượng. Datasets Guide ghi tối thiểu 100 dòng, tốt nhất trên 1.000 dòng. What Model Should I Use? ghi dưới 300 dòng thì dùng instruct model. Các trang không thống nhất — xem hộp cảnh báo ở trang [Cần bao nhiêu dữ liệu](/du-lieu/#so-luong).
...
**Nguồn:** https://unsloth.ai/docs/get-started/fine-tuning-llms-guide/datasets-guide, https://unsloth.ai/docs/get-started/fine-tuning-llms-guide, https://unsloth.ai/docs/basics/chat-templates, https://unsloth.ai/docs/basics/vision-fine-tuning, https://unsloth.ai/docs/new/studio/start

## Lỗi thường gặp khi chuẩn bị dữ liệu

Phần lớn lỗi nằm ở chat template, tiếp theo là vài chi tiết khi gộp hội thoại và xử lý dữ liệu vision.
...
**Nguồn:** https://unsloth.ai/docs/basics/chat-templates, https://unsloth.ai/docs/get-started/fine-tuning-llms-guide/tutorial-how-to-finetune-llama-3-and-use-in-ollama, https://unsloth.ai/docs/basics/vision-fine-tuning, https://unsloth.ai/docs/get-started/fine-tuning-llms-guide/datasets-guide

## Đọc tiếp

- [Export và deploy](/export-deploy/) — phần kế tiếp: sau khi train, xuất model ra các định dạng để chạy.
- [Quy trình từng bước](/fine-tuning/quy-trinh) — đưa dataset đã rà vào quy trình fine-tune đầy đủ.
- [Cần bao nhiêu dữ liệu](/du-lieu/#so-luong) — xem lại các mốc số dòng và hộp "Docs chưa thống nhất".
