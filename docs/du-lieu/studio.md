---
title: Làm dữ liệu trong Studio
...
# Làm dữ liệu trong Studio

Unsloth Studio là giao diện web chạy local để fine-tune mà không cần viết code. Trang này giúp bạn nạp dataset vào Studio và dùng Data Recipes để biến tài liệu của mình (PDF, CSV...) thành dataset train được.

::: tip Tóm tắt
- **Dùng khi:** bạn train bằng Studio và cần nạp dataset, hoặc chỉ có tài liệu thô và muốn Studio dựng dataset giúp.
- **Kết quả:** biết cách nạp dataset từ Hugging Face Hub hoặc file local, chọn format, split, slice, gán cột; và biết dựng một recipe từ các block để sinh dataset.
- **Nên biết trước:** [Định dạng dữ liệu](/du-lieu/dinh-dang), [Cài đặt và phần cứng](/cai-dat).
:::

## Nạp dataset ở bước Dataset

Nếu train bằng Studio, bạn nạp dataset ở bước Dataset. Bước này có hai tab:
...
## Data Recipes: biến tài liệu thành dataset {#data-recipes}
...
Một recipe đi từ dựng block, kiểm tra, xem thử vài dòng, rồi mới chạy toàn bộ:
...
- [Sinh dữ liệu tổng hợp](/du-lieu/synthetic) — trang kế tiếp: các cách khác để sinh dữ liệu bằng LLM, kèm prompt mẫu.
- [Checklist chuẩn bị dữ liệu](/du-lieu/checklist) — rà lại dataset, gồm các mục riêng cho Studio, trước khi train.
- [Định dạng dữ liệu](/du-lieu/dinh-dang) — hiểu rõ các format `alpaca`, `chatml`, `sharegpt` mà Studio yêu cầu chọn.
