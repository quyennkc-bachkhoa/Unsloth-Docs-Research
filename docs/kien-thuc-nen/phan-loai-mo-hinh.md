...
Trang này giúp bạn đọc tên một model trên Hugging Face và biết nó thuộc loại nào: lớn hay nhỏ, đã được dạy làm theo chỉ dẫn hay chưa, có nhận ảnh hoặc âm thanh không, sinh văn bản hay trả về vector. Nên đọc trước khi chọn model trong [Model catalog](/model-catalog) hoặc bắt đầu [Fine-tuning](/fine-tuning).
...
**Khái niệm.** LLM (Large Language Model, mô hình ngôn ngữ lớn) là tên gọi chung cho các transformer language model được huấn luyện trên lượng dữ liệu lớn và thường có rất nhiều tham số; glossary của Hugging Face lấy ví dụ GPT-3 với 175 tỷ tham số nhưng **không đặt ngưỡng**. **[Nguồn ngoài]** https://huggingface.co/docs/transformers/glossary. SLM (Small Language Model, mô hình ngôn ngữ nhỏ) là cách gọi các model ít tham số hơn. Trong các nguồn đã duyệt (docs Unsloth, HF LLM Course, Transformers glossary), **không có ngưỡng chính thức** về số tham số để tách LLM và SLM, nên trang này không tự đặt ngưỡng.
...
**Ảnh hưởng khi dùng Unsloth.** Thay vì gắn nhãn LLM/SLM, hãy nhìn số tham số (và số tham số kích hoạt với MoE), vì nó quyết định VRAM cần có. Docs chỉ tới bảng VRAM theo số tham số trong trang Unsloth Requirements. Xem [Tham số & bộ nhớ](/kien-thuc-nen/tham-so-va-bo-nho) và [Dense & MoE](/kien-thuc-nen/dense-va-moe).
...
**Khái niệm.**
- **Base model** (model nền) là bản chỉ mới pre-train: học đoán token tiếp theo trên kho văn bản khổng lồ, chưa được dạy trả lời theo chỉ dẫn. Base model "viết tiếp" văn bản chứ không "trả lời" câu hỏi.
- **Instruct / chat model** là base model đã qua post-training (huấn luyện sau), ví dụ fine-tune trên dữ liệu hội thoại, để làm theo chỉ dẫn và chat. Theo docs Unsloth, instruct model dùng ngay được mà không cần fine-tune, và làm việc với chat template dạng hội thoại (ChatML, ShareGPT).
...
Bản không có hậu tố (`Qwen3-8B`) mới là bản đã post-train; model card của nó ghi được xây từ `Qwen3-8B-Base`. **[Nguồn ngoài]** https://huggingface.co/Qwen/Qwen3-8B-Base, https://huggingface.co/Qwen/Qwen3-8B

**Ảnh hưởng khi dùng Unsloth.** Docs Unsloth khuyên:
- **Bắt đầu với instruct model**: fine-tune trực tiếp bằng chat template hội thoại được và cần ít dữ liệu hơn base model. Base model hợp với template kiểu instruction (Alpaca, Vicuna) và thường **không** hỗ trợ sẵn chat template hội thoại.
...
- Nếu được, fine-tune thử cả hai rồi so sánh đầu ra.
...
Hai câu không hẳn mâu thuẫn (một câu nói về điểm bắt đầu, một câu dựa vào lượng dữ liệu), nhưng docs không nói rõ nên ưu tiên câu nào khi bạn là người mới **và** có trên 1.000 dòng.
:::
...
**Khái niệm.** Model text chỉ nhận và sinh văn bản. Model multimodal (đa phương thức) kết hợp văn bản với loại đầu vào khác như ảnh hoặc âm thanh **[Nguồn ngoài]** https://huggingface.co/docs/transformers/glossary. VLM (Vision Language Model, mô hình ngôn ngữ thị giác) là loại nhận ảnh + chữ và sinh ra chữ. Transformer thị giác cắt ảnh thành các mảnh (patch) rồi đưa vào model như một chuỗi, nên ảnh cũng chiếm token trong context **[Nguồn ngoài]** https://huggingface.co/docs/transformers/glossary. Xem [Token & context](/kien-thuc-nen/token-va-context).

**Ví dụ.**
- **Vision:** Qwen3-VL có khả năng vision, video, OCR, có cả bản instruct và thinking. Model card Gemma 3 ghi họ model này nhận văn bản + ảnh và sinh văn bản; mỗi ảnh chuẩn hóa về 896 × 896 và mã hóa thành 256 token **[Nguồn ngoài]** https://huggingface.co/google/gemma-3-4b-it.
- **Audio đầu vào:** Gemma 4 E2B, E4B và 12B nhận văn bản, ảnh và âm thanh (26B-A4B và 31B chỉ nhận văn bản + ảnh); âm thanh tối đa 30 giây. E2B được gợi ý cho ASR (nhận dạng giọng nói) và dịch giọng nói.
- **Audio đầu ra (TTS) và STT:** Unsloth hỗ trợ fine-tune các model TTS (text-to-speech, chuyển chữ thành giọng nói) như Orpheus-TTS (3B), Sesame-CSM (1B), Spark-TTS (0.5B), Llasa-TTS (1B), Oute-TTS (1B), và STT (speech-to-text) Whisper Large V3. Theo docs, Orpheus là model giọng nói dựa trên Llama, sinh thẳng **audio token** rồi giải mã thành sóng âm.

**Ảnh hưởng khi dùng Unsloth.**
- Model vision dùng `FastVisionModel` và `UnslothVisionDataCollator` thay cho `FastLanguageModel`. Có thể chọn chỉ fine-tune phần vision hoặc phần ngôn ngữ (`finetune_vision_layers`, `finetune_language_layers`, mặc định bật cả hai).
- Dataset vision có dạng tin nhắn chứa cả `{"type": "text", ...}` và `{"type": "image", ...}`. Docs khuyên ảnh cùng kích thước, trong khoảng 300–1000px để train không quá lâu và tốn tài nguyên.
- Docs "What model should I use" ghi: nếu train trên ảnh thì chọn vision model (ví dụ Llama 3.2 Vision); với dataset code thì chọn model chuyên code (ví dụ Qwen Coder 2.5).
...
**Khái niệm.**
- **LLM sinh văn bản** thường là decoder model (còn gọi là autoregressive): đọc từ trái sang phải và đoán token tiếp theo; đầu ra là **token**, lặp lại cho tới khi đủ câu trả lời. Ví dụ: họ Llama, Gemma, DeepSeek-V3. **[Nguồn ngoài]** https://huggingface.co/learn/llm-course/chapter1/6
- **Encoder model** (còn gọi là autoencoding) nhìn toàn bộ câu cùng lúc và biến đầu vào thành một biểu diễn số cô đọng gọi là **embedding**; thường được pre-train bằng masked language modeling (che một số token rồi đoán lại). Ví dụ: BERT, ModernBERT. **[Nguồn ngoài]** https://huggingface.co/docs/transformers/glossary, https://huggingface.co/learn/llm-course/chapter1/6
- **Embedding model** trả về một **vector** (dãy số) đại diện cho nghĩa của cả câu hoặc đoạn văn. Hai đoạn gần nghĩa có vector gần nhau, nên dùng để tìm kiếm ngữ nghĩa, RAG, phân cụm.

Điểm phân biệt cốt lõi là **đầu ra** (vector hay token), không phải kiến trúc. Nhiều embedding model là encoder-only, nhưng không phải tất cả: Qwen3-Embedding được xây từ model nền dense của họ Qwen3 và lấy vector từ trạng thái ẩn của **token cuối** (last token pooling). **[Nguồn ngoài]** https://huggingface.co/Qwen/Qwen3-Embedding-0.6B

**Ví dụ.** Qwen3-Embedding-0.6B: 0.6B tham số, context 32K, vector tối đa 1024 chiều (người dùng chọn được từ 32 đến 1024) **[Nguồn ngoài]** https://huggingface.co/Qwen/Qwen3-Embedding-0.6B. Docs Unsloth đưa ví dụ: hai tiêu đề "Google launches Pixel 10" và "Qwen releases Qwen3" có thể được coi là giống nhau nếu chỉ phân loại chủ đề "Tech", nhưng không giống nhau nếu làm tìm kiếm ngữ nghĩa. Fine-tune embedding giúp model học đúng kiểu "giống nhau" mà bài toán của bạn cần.

**Ảnh hưởng khi dùng Unsloth.**
- Embedding dùng lớp `FastSentenceTransformer` (dựa trên SentenceTransformers), không dùng `FastLanguageModel`. Khi load để chạy, **bắt buộc** truyền `for_inference=True`:
...
- Unsloth hỗ trợ tốt nhất các `SentenceTransformer` encoder-only có file `modules.json`; model không có file này chỉ được hỗ trợ hạn chế (Unsloth tự gán pooling mặc định), nên cần kiểm tra lại vector đầu ra nếu dùng head hoặc pooling tùy biến.
- Kết quả dùng lại được với transformers, LangChain, sentence-transformers, TEI, vLLM, llama.cpp, pgvector, FAISS và các framework RAG.
...
**Khái niệm.** Tên repo trên Hugging Face thường ghép từ: họ model + phiên bản + kích thước + hậu tố cho biết giai đoạn huấn luyện, khả năng, định dạng. **Không có chuẩn chung giữa các hãng**: cùng là bản đã post-train, Qwen3 không có hậu tố (`Qwen3-8B`), Gemma dùng `-it`, Qwen2.5-VL dùng `-Instruct`. Luôn đọc model card để chắc chắn.
...
**Ảnh hưởng khi dùng Unsloth.**
- Catalog Unsloth ghi: dùng **GGUF** để chạy trong Unsloth Desktop, llama.cpp; dùng **Instruct (4-bit)** safetensors để chạy hoặc fine-tune qua Unsloth. Đừng đưa repo `-GGUF` vào `FastLanguageModel` để fine-tune. **[Nhận định]** suy ra từ cách catalog tách hai cột này.
- Muốn đổi model thì chỉ cần đổi `model_name` cho khớp tên trên Hugging Face, ví dụ `unsloth/llama-3.1-8b-unsloth-bnb-4bit`.
- Một số tiền tố như `E2B`/`E4B` của Gemma 4 được docs Unsloth dùng (kèm ghi chú "Dense + PLE") nhưng không giải thích chữ "E" nghĩa là gì: **cần kiểm tra lại** trong model card chính thức của Google.
